...
func (app *ScrapemateApp) getFetcher() (scrapemate.HTTPFetcher, error) {
	...
	case true:
		var jsFetcher scrapemate.HTTPFetcher

		jsFetcher, err = app.getJSFetcher(rotator)
		if err != nil {
			return nil, err
		}

		// 混合抓取：搜索滚动走浏览器，详情页/邮箱站走纯HTTP（失败自动回退浏览器）
		httpFetcher = newMuxFetcher(jsFetcher, rotator)
	default:
