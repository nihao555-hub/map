...
打开 `/discover`。智能引擎和发开发信用深色窄栏；地图获客已用 git 回退到 `main` 上那一版（无窄侧栏）。智能引擎**不是地图搜店**，页面上可选两种模式：

1. **私信模式**（默认）：关键词 → 抖音 / TikTok 等社媒店主或真人公开主页 → 点击「打开主页 / 去私信」在**右侧预览栏**渲染主页摘要和草稿。官方页大多禁止 iframe 内嵌，可再点「在官方页打开」。系统不代发。
