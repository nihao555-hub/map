...
`/directory` 翻看本地库：GLEIF 法律名（无官网/社媒）和已入库的抖音/TikTok 企业号主页分开统计。要灌短视频企业号：`make ingest-short-video`；两小时窗口用 `make ingest-short-video-fast`（Wikidata 全量号 + 东南亚→中东→欧美，不是平台全库）。
